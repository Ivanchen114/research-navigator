...
const FOUR_LAYERS = [
    ...
    { name: '呼應', color: '#DC2626', task: '回頭回答你最初的研究問題（你問的問題答了沒）', ai: '人寫內容和邏輯，AI 只能潤飾文句', scaffold: '本研究原本想了解___。根據分析結果，本研究的答案是___。但這個結論只能說明___，無法確定___。', star: true },
...
2. 優化詮釋層，補充可能遺漏的原因。
3. 呼應層我已經寫好內容和邏輯，請只幫我潤飾文句，不要改變意思。
4. 列出問卷研究常見的三種研究限制供我參考。
5. 最後把第 1-4 點的建議整理成一份「逐層修改清單」（描述／詮釋／呼應／限制各要改什麼），讓我照著自己修改——不要替我整合成完整的結論段落，定稿由我自己完成。`,
...
5. 最後把第 1-4 點的建議整理成一份「逐層修改清單」（描述／詮釋／呼應／限制各要改什麼），讓我照著自己修改——不要替我整合成完整的結論段落，定稿由我自己完成。`,
        tips: ['有差異不等於有效：先確認干擾變因', '用「顯示差異」不是「證明效果」', '「被看就不一樣」效應（學術叫霍桑效應）：受試者知道自己在被觀察，行為可能就不像平常'],
    },
    observation: {
        label: '👀 觀察組',
        prompt: `我的研究主題是＿＿＿，研究問題是＿＿＿。
...
const DEMO_EXAMPLE = {
    ...
    layers: [
        ...
        { name: '呼應', color: '#DC2626', text: '本研究原本想了解手機使用時間是否與年級有關。根據分析結果，年級越高使用時間越短，兩者有相關性。但這個結論只能說明兩者有相關，無法確定是年級本身還是其他因素（如補習頻率）造成差異。' },
        { name: '批判', color: '#059669', text: '本研究樣本僅限兩班 60 人，且為自填問卷，受訪者可能因為「想顯得更好」而沒填真實想法（這種傾向學術上叫社會期許偏差），結論不宜推論至全體高中生。若要更完整回答這個問題，未來可擴大樣本並加入訪談交叉驗證。' },
    ],
};

const EXPORT_FIELDS = [
    { key: 'w15-three-column', label: '三欄自查（能直接說／要謹慎說／不能說）', question: '寫結論前把每句話分類進三欄' },
            ...
            {current ? (
                <div className="flex flex-col gap-4">
                    ...
                    <div className="p-4 rounded-[var(--radius-unified)] border border-[#FCD34D] bg-[#FFFBEB]">
                        <p className="text-[12px] text-[#92400E] font-bold mb-2">🧠 人工判斷取捨提醒</p>
...
const W15PageContent = () => {
    ...
    const WARMUP_ANSWERS = { 1: '描述', 2: '詮釋', 3: '呼應', 4: '批判' };
    const WARMUP_HINTS = {
        1: '有具體數字（61%、78%）——這是在「報告事實」',
        2: '「可能反映」「形成」——這是在「解讀意義」',
        3: '「部分回答了研究問題」＋「但只能說明」——呼應＋邊界，是最重要的一層',
        4: '樣本限制、問卷偏誤——這是在「說清楚研究做不到什麼」',
    };
    const warmupAllDone = Object.values(warmupSel).every(v => v !== '');
    const handleWarmup = (n, layer) => {
        if (warmupRevealed) return;
        setWarmupSel(prev => ({ ...prev, [n]: prev[n] === layer ? '' : layer }));
    };
    ...
    const steps = [
        {
            ...
            content: (
                <div className="flex flex-col gap-6 prose-zh">
                    <StepBriefing
                        lines={[
                            { label: '節奏', text: '跟老師做（第一節）' },
                            { label: '學', text: '四層結論架構：描述（事實）→ 詮釋（意義）→ 呼應（回答研究問題）→ 批判（限制）' },
                            { label: '做', text: '找到你的研究方法對應的呼應層範例' },
                            { label: '練', text: '暖身練習 2 題，確認四層分得清楚' },
                        ]}
                    />
                    <div className="p-5 rounded-[var(--radius-unified)] border border-[var(--border)] bg-[var(--paper-warm)]">
                        <div className="flex items-center gap-2 mb-2">
                            <ContentTypeChip type="學" />
                            <p className="text-[14px] font-bold text-[var(--ink)]">🎓 升級！從局部到全局</p>
                        </div>
                    ...
                    {/* 名詞升級：W14 推論 → W15 詮釋 — 名詞白話化，深度補充 */}
                    <DepthBlock title="名詞升級：推論→詮釋">
                    <div className="p-4 rounded-[var(--radius-unified)] border-2 border-[#DDD6FE] bg-[#F5F3FF]">
                        ...
                        <p className="text-[12px] text-[#4C1D95] leading-relaxed mb-2">
                            ...
                            今天開始你會看到「四層」用學術名：<strong>描述 / 詮釋 / 呼應 / 批判</strong>。
                        </p>
                    ...
                    <div className="grid md:grid-cols-2 gap-3 text-[12px]">
                        <div className="p-3 rounded-[var(--radius-unified)] bg-white border-2 border-[#FCA5A5]">
                            <p className="font-bold text-[#DC2626] mb-1">⭐ 第三層：呼應</p>
                            <p className="text-[var(--ink-mid)] leading-relaxed">直接回答你最初的研究問題——你問的問題，答了沒？</p>
                        </div>
                        <div className="p-3 rounded-[var(--radius-unified)] bg-white border-2 border-[#6EE7B7]">
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-3">
                            <ContentTypeChip type="學" />
                            <p className="text-[13px] font-bold text-[var(--ink)]">📐 四層結論架構</p>
                        </div>
                    ...
                    <div className="p-4 rounded-[var(--radius-unified)] border border-[#FCA5A5] bg-[#FEF2F2]">
                        <div className="flex items-center gap-2 mb-1">
                            <ContentTypeChip type="學" />
                            <p className="text-[13px] text-[#DC2626] font-bold">⭐ 呼應層為什麼特別？</p>
                        </div>
                        <p className="text-[12px] text-[#991B1B] leading-relaxed mb-3">
                            AI 可以幫你把呼應層的文句說得更通順，但<strong>內容和邏輯只能由你來寫</strong>。因為只有你知道你當初為什麼要做這個研究、你的問題問的是什麼。
                        </p>
                    ...
                    <p className="text-[12px] text-[var(--ink-mid)] leading-relaxed">
                        呼應層的寫法跟你的研究方法有關——問卷、訪談、實驗、觀察、文獻各有範例。展開看看哪一個最像你們組。
                    </p>
                    <DepthBlock title="看完整範例">
                    <div className="p-4 rounded-[var(--radius-unified)] border border-[var(--border)] bg-white">
                        <p className="text-[13px] font-bold text-[var(--ink)] mb-3">📌 呼應層範例：不同方法怎麼寫？</p>
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <p className="text-[13px] font-bold text-[var(--ink)]">✏️ 暖身練習：判斷下面四句話各屬哪一層</p>
                        </div>
                        <p className="text-[12px] text-[var(--ink-mid)] mb-3">主題：手機使用與學習成績關係的研究（N=85）——每句點選一個層次</p>

                        {/* 層次顏色說明列 */}
                        <div className="flex flex-wrap gap-2 mb-4">
                            {FOUR_LAYERS.map(l => (
                                <span key={l.name} className="text-[11px] font-bold px-2 py-0.5 rounded text-white" style={{ background: l.color }}>{l.name}</span>
                            ))}
                        </div>

                        <div className="flex flex-col gap-3">
                            {[
                                { n: 1, text: '根據圖一，N=85 份問卷中，61% 的學生每天使用手機超過 4 小時，其中 78% 主要用途為短影音與社交。' },
                                { n: 2, text: '這可能反映出學生放學後的娛樂模式已高度依賴手機，形成分心的習慣迴路。' },
                                { n: 3, text: '本研究初步發現手機使用時間與學業成績之間有負相關趨勢，部分回答了研究問題；但只能說明兩者相關，無法確定是手機造成成績下滑，還是相反。' },
                                { n: 4, text: '本研究樣本僅本校高一生 85 人，不宜推論至其他年級或其他學校；問卷亦為自填，存在社會期許偏誤。' },
                            ].map(({ n, text }) => {
                                const sel = warmupSel[n];
                                const ans = WARMUP_ANSWERS[n];
                                const correct = warmupRevealed ? sel === ans : null;
                                const ansColor = FOUR_LAYERS.find(l => l.name === ans)?.color ?? '#888';
                                return (
                                    <div key={n} className="rounded-[var(--radius-unified)] border bg-white overflow-hidden"
                                        style={{ borderColor: warmupRevealed ? (correct ? '#059669' : '#DC2626') : 'var(--border)', borderWidth: warmupRevealed ? 2 : 1 }}>
                                        {/* 句子 */}
                                        <div className="p-3 px-4 text-[12.5px] text-[var(--ink)] leading-relaxed border-b border-[var(--border)]">
                                            <span className="font-bold text-[var(--accent)] mr-1">第 {n} 句：</span>{text}
                                        </div>
                                        {/* 選項列 */}
                                        <div className="flex flex-wrap gap-2 p-3 px-4">
                                            {FOUR_LAYERS.map(l => {
                                                const isSelected = sel === l.name;
                                                return (
                                                    <button
                                                        key={l.name}
                                                        onClick={() => handleWarmup(n, l.name)}
                                                        className="text-[12px] font-bold px-3 py-1 rounded-full border-2 transition-all"
                                                        style={isSelected
                                                            ? { background: l.color, borderColor: l.color, color: 'white' }
                                                            : { background: 'white', borderColor: l.color, color: l.color }}
                                                    >
                                                        {l.name}
                                                    </button>
                                                );
                                            ...
                                            {/* 結果提示 */}
                                            {warmupRevealed && (
                                                <span className="ml-auto text-[12px] font-bold flex items-center gap-1.5">
                                                    {correct
                                                        ? <span className="text-[#059669]">✓ 正確</span>
                                                        : <span className="text-[#DC2626]">✗ 答案：<span className="px-1.5 py-0.5 rounded text-white text-[11px]" style={{ background: ansColor }}>{ans}</span></span>
                                                    }
                                                </span>
                                            )}
                                        </div>
                                        {/* 答案提示說明 */}
                                        {warmupRevealed && (
                                            <div className="px-4 pb-3 text-[11.5px] leading-relaxed"
                                                style={{ color: correct ? '#059669' : '#DC2626' }}>
                                                💡 {WARMUP_HINTS[n]}
                                            </div>
                                        )}
                                    </div>
                        ...
                        <div className="mt-4 flex gap-3 items-center">
                            {!warmupRevealed ? (
                                <button
                                    ...
                                    style={warmupAllDone
                                        ...
                                        : { background: 'var(--border)', color: 'var(--ink-light)', cursor: 'not-allowed' }}
                                >
                                    {warmupAllDone ? '✅ 查看答案' : `查看答案（還有 ${4 - Object.values(warmupSel).filter(v => v).length} 句未選）`}
                                </button>
                            ) : (
                                <button
                                    onClick={() => { setWarmupSel({ 1: '', 2: '', 3: '', 4: '' }); setWarmupRevealed(false); }}
                                    className="px-4 py-2 rounded-lg text-[13px] font-bold border border-[var(--border)] text-[var(--ink-mid)] hover:text-[var(--ink)] transition-colors"
                                >
                                    🔄 重新作答
                                </button>
                            )}
                            {warmupRevealed && (
                                <span className="text-[12px] text-[var(--ink-mid)]">
                                    {Object.keys(WARMUP_ANSWERS).filter(n => warmupSel[n] === WARMUP_ANSWERS[n]).length} / 4 正確
                                </span>
                            )}
                        </div>
                    </div>

                    {/* 雷 #3 改寫練習 — 呼應層剛教完、觀念最新鮮，立刻練（個人繳交項）*/}
                    <div className="flex items-center gap-2 mb-1">
                        <ContentTypeChip type="做" />
                        <p className="text-[12px] font-bold text-[var(--ink-mid)]">雷 #3 改寫練習（個人繳交項）</p>
                    </div>
                    <TrapRewritePractice
                        trapNumber={3}
                        stage="W15"
        ...
        },
        {
            ...
            content: (
                <div className="flex flex-col gap-6 prose-zh">
                    <StepBriefing
                        lines={[
                            { label: '節奏', text: '個人，課堂 or 課後' },
                            { label: '做', text: '在小組共筆 / docx 把四層結論初稿寫一輪' },
                            { label: '注意', text: '粗糙沒關係，重點是不要停在空白頁' },
                        ]}
                    />
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            <ContentTypeChip type="做" />
                            <p className="text-[13px] font-bold text-[var(--ink)]">📊 三欄結論鷹架（先分類「能說 / 謹慎說 / 不能說」再動筆）</p>
                        </div>
                    ...
                    <div className="rounded-[var(--radius-unified)] border-2 border-[var(--accent)] bg-[#F8F8FB] p-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11.5px] leading-relaxed mb-3">
                            ...
                            <div className="bg-white border border-[#DDD6FE] rounded p-2.5">
                                <p className="font-bold text-[#7C3AED] mb-0.5">🟣 詮釋</p>
                                <p className="text-[var(--ink-mid)] m-0">這可能代表什麼？</p>
                            </div>
                            <div className="bg-white border border-[#FCA5A5] rounded p-2.5">
                                <p className="font-bold text-[#DC2626] mb-0.5">🔴 呼應</p>
                                <p className="text-[var(--ink-mid)] m-0">這回答了我的研究問題，因為……</p>
                            </div>
                            <div className="bg-white border border-[#86EFAC] rounded p-2.5">
                                <p className="font-bold text-[#059669] mb-0.5">🟢 批判</p>
            ...
            content: (
                <div className="flex flex-col gap-6 prose-zh">
                    <StepBriefing
                        lines={[
                            { label: '節奏', text: '個人，課堂 or 課後' },
                            { label: '學', text: '相關 ≠ 因果（最容易踩的雷）' },
                            { label: '做', text: '對著初稿逐層自查，修正後把最終版填到下方必繳格子' },
                        ]}
                    />
                    <div className="p-4 rounded-[var(--radius-unified)] border border-[var(--border)] bg-[var(--paper-warm)]">
                        <div className="flex items-center gap-2 mb-1">
                            <ContentTypeChip type="做" />
                            <p className="text-[14px] font-bold text-[var(--ink)]">🔍 對著自己初稿做四層自查</p>
                        </div>
                        <p className="text-[12px] text-[var(--ink-mid)] leading-relaxed">
                            Step 2 寫好了初稿，<strong>不用急著找 AI</strong>——先自己重讀一遍。
                            初學者最常踩的雷不是「文句不漂亮」，而是「描述層偷渡因果」「詮釋層用沒問過的變項」「呼應層忘了回答自己的研究問題」。這些自己就抓得到。
                        </p>
                    </div>
                    ...
                    <div className="p-4 rounded-[var(--radius-unified)] border border-[#FCD34D] bg-[#FFFBEB]">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <p className="text-[12px] text-[#92400E] font-bold">⚠️ 注意：若下一步要用 AI 檢核，記得先清個資</p>
                        </div>
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-3">
                            <ContentTypeChip type="做" />
                            <p className="text-[13px] font-bold text-[var(--ink)]">📋 四層自查清單（對著你 Step 2 寫的初稿一條條勾）</p>
                        </div>
                        <div className="flex flex-col gap-3">
                            <Checklist
                                dataKey="w15-check-desc"
                                prompt="🔵 描述層"
                                items={[
                                    '所有數字都跟原始資料對得上',
                                    '量詞精準（不寫「許多」「大多數」這類模糊詞）',
                                    '沒有偷渡因果（「因為／導致」是詮釋層的事）',
                                    'N 值有清楚交代',
                                ]}
                            />
                            <Checklist
                                dataKey="w15-check-interp"
                                prompt="🟣 詮釋層"
                                items={[
                                    '解釋的「原因」沒有用問卷沒問過的變項',
                                    '訪談組：沒有過度詮釋（沒說超過受訪者實際說的）',
                                    '有區分「相關」與「因果」',
                                    '推論有資料支持，不是個人猜測',
                                ]}
                            />
                            <Checklist
                                dataKey="w15-check-respond"
                                prompt="🔴 呼應層（核心）"
                                items={[
                                    '有直接回答最初的研究問題',
                                    '結論強度合理（沒寫「證明」「證實」）',
                                    '「能說明什麼／無法確定什麼」兩部分都寫了',
                                    '範圍合理（沒有過度推論到研究範圍外的對象）',
                                ]}
                            />
                            <Checklist
                                dataKey="w15-check-crit"
                                prompt="🟢 批判層"
                                items={[
                                    '至少列了 2 個研究限制',
                                    '限制有具體（例：樣本只 N=60／自填問卷有社會期許）',
                                    '有說明結論不宜推論到哪些對象',
                                    '提了未來可改進方向',
                                ]}
                            />
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            <ContentTypeChip type="做" />
                            <p className="text-[13px] font-bold text-[var(--ink)]">✏️ 自查紀錄（對著清單修改後，寫下你改了什麼）</p>
                        </div>
                        <ThinkRecord
                    ...
                    <div className="p-4 rounded-[var(--radius-unified)] border-2 border-[var(--accent)]">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <p className="text-[14px] font-bold text-[var(--accent)]">📝 四層結論最終版（必繳 · 唯一最終欄位）</p>
                        </div>
...
【批判】但這個結論不能說……`}
                    ...
                    <div className="p-4 rounded-[var(--radius-unified)] border border-[var(--border)]">
                        ...
                        <div className="text-[12px] text-[var(--ink-mid)] leading-relaxed flex flex-col gap-1">
                            ...
                            <span><strong>第五章 討論與結論</strong> → 放【詮釋層】＋【呼應層】＋【批判層】</span>
                        </div>
                    </div>
                </div>
            ),
        },
        {
            ...
            content: (
                <div className="flex flex-col gap-6 prose-zh">
                    <StepBriefing
                        lines={[
                            { label: '做', text: '四層結論丟給 AI 當嚴格教練找盲點，可跳過' },
                        ]}
                    />
                    {/* W15 AI 一句話規則 — 結論責任收束週的底線 */}
                    <div className="rounded-[var(--radius-unified)] border-l-4 border-[#DC2626] bg-[#FEF2F2] p-4">
                        <div className="flex items-center gap-2 mb-1.5">
                            <ContentTypeChip type="注意" />
                            <p className="text-[13px] font-bold text-[#991B1B] leading-[1.85] m-0">
                                🚧 W15 的 AI 底線：AI 可以幫你<strong>檢查結論有沒有跳太遠</strong>、可以幫你<strong>整理結論骨架</strong>；但最後的結論段落，要由研究者自己整合。
                            </p>
                        </div>
                        <p className="text-[11.5px] text-[#7F1D1D] leading-[1.8] mt-1.5 mb-0">
                            W13 整理、W14 圖說，AI 能做局部技術活；但 W15 的「結論」是整個研究的責任收束——AI 是結論<strong>審查員</strong>，不是<strong>代筆者</strong>。
                        </p>
                    </div>
                    ...
                    <div className="p-5 rounded-[var(--radius-unified)] border-2 border-[var(--accent)] bg-[#F8F8FB]">
                        <div className="flex items-center gap-2 mb-2">
                            <ContentTypeChip type="學" />
                            <p className="text-[15px] font-bold text-[var(--accent)]">🤖 AI 是嚴格教練 · 你可以選擇不用</p>
                        </div>
                    ...
                    {/* AI 協作三原則（W15 角色：嚴格教練）— 深度補充 */}
                    <DepthBlock title="AI 使用提醒">
                        <AICollaborationPrinciples week="15" role="critic" compact={true} />
                    </DepthBlock>

                    {/* 個資鐵規 */}
                    <DepthBlock title="⚠️ 餵 AI 前：個資清除鐵規">
                        <div className="text-[12px] text-[var(--ink-mid)] leading-relaxed space-y-1">
                            <p>你在 W11 簽了知情同意書，承諾保護受訪者的個人資料。貼給 AI 前必須先清除。</p>
                            <p className="font-bold text-[var(--ink)]">📋 問卷：刪除所有姓名、學號、Line 暱稱再貼</p>
                            <p className="font-bold text-[var(--ink)]">🎤 訪談：受訪者真實姓名改為代號（A、B、C）再貼</p>
                            <p className="text-[#DC2626] font-bold mt-1">沒有完成個資清除就貼給 AI，是違反你對受訪者的承諾。</p>
                        </div>
                    </DepthBlock>
                    ...
                    {w15AiMode === 'teach' && (
                        <div className="rounded-[var(--radius-unified)] border-2 border-[#86EFAC] bg-[#F0FDF4] p-5 space-y-3">
                            ...
                            <PromptBlock text={`我在做研究，需要寫「四層結論」（描述、詮釋、呼應、批判），但我完全不知道怎麼寫。
                    ...
                    {w15AiMode === 'verify' && (
                        <>
                            ...
                            <div className="p-4 rounded-[var(--radius-unified)] border-2 border-[#DC2626] bg-[#FEF2F2]">
                                <p className="text-[13px] font-bold text-[#991B1B] mb-2">🚦 AI 改稿四層紅線檢核（批判層由你自寫，AI 不介入）</p>
                                <p className="text-[11.5px] text-[#7F1D1D] leading-relaxed mb-3">
                                    AI 會把建議混在一起送回來，你必須<strong>逐條判斷</strong>。以下是 AI 可介入的描述／詮釋／呼應 三層各自紅線；批判層（研究限制）只由你自己寫，本卡不列。
                                </p>
                                <div className="grid md:grid-cols-3 gap-2">
                                    ...
                                    <div className="bg-white border border-[#DC2626] rounded-[6px] p-3">
                                        <p className="text-[12px] font-bold text-[#991B1B] mb-1">🔴 呼應層（核心）</p>
                                ...
                                {/* 常見失誤具體例：3 個學生最容易踩的紅線 — 深度補充 */}
                                <DepthBlock title="常見錯誤">
                                    <div className="space-y-2 text-[11.5px] text-[#7F1D1D] leading-relaxed">
                                        <p className="text-[11.5px] font-bold text-[#991B1B]">📋 學生最容易踩的 3 個紅線</p>
                                        <div>
                            ...
                            <div className="bg-[var(--paper-warm)] border border-[var(--border)] rounded-[var(--radius-unified)] p-3 flex items-center justify-between gap-3">
                                <p className="text-[12px] text-[var(--ink-mid)] leading-relaxed">
                                    💡 想看更進階的 5 法檢核 prompt？回 <strong className="text-[var(--ink)]">資料分析檢核站</strong>看 Step 2-5 進階版——自學用，不影響本週繳交。
                                </p>
                                <a
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-2">
                                    <ContentTypeChip type="做" />
                                    <p className="text-[13px] font-bold text-[var(--ink)]">✏️ AI 建議紀錄（跑完後填）</p>
                                </div>
                                <ThinkRecord
                                    dataKey="w15-ai-feedback"
                                    prompt="AI 提供了哪些建議？分別記錄描述層、詮釋層、呼應層潤飾、研究限制的建議重點。"
                                    scaffold={['描述層建議：...', '詮釋層建議：...', '呼應層潤飾：...', '研究限制建議：1.__ 2.__ 3.__']}
                                />
                            </div>
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-2">
                                    <ContentTypeChip type="做" />
                                    <p className="text-[13px] font-bold text-[var(--ink)]">✏️ 判斷取捨紀錄（逐條寫理由）</p>
                                </div>
                                <ThinkRecord
                                    dataKey="w15-judge-table"
                                    prompt="逐條記錄 AI 的建議和你的判斷取捨（採納/不採納/修改後採納），並說明原因。"
                                    scaffold={['建議1：___。判斷：___。原因：...', '建議2：___。判斷：___。原因：...', '建議3：___。判斷：___。原因：...']}
                ...
                <div className="flex flex-col gap-6 prose-zh">
                    ...
                    <div className="bg-white border-2 border-[#10B981] rounded-[var(--radius-unified)] overflow-hidden">
                        ...
                        <div className="p-4 space-y-4">
                            ...
                            <div className="border-l-4 border-[#0284C7] bg-[#F0F9FF] rounded-r-[6px] p-3">
                                <p className="font-bold text-[13.5px] text-[#075985] mb-1.5">一、小組作業｜研究結論章成果</p>
                                <ul className="text-[12.5px] text-[#0C4A6E] leading-[1.85] list-disc pl-5 space-y-1">
                                    <li><strong>研究結論章初稿</strong>（四層整合：描述／詮釋／呼應／批判）</li>
                                    <li>放回組內研究報告對應章節，W16 接報告與海報整合</li>
                                </ul>
                                <p className="text-[11.5px] text-[#0C4A6E] italic mt-2 pt-2 border-t border-[#0284C7]/30">
                                    💡 評分眼光：<strong>結論有沒有回答研究問題、有沒有把相關寫成因果、研究限制有沒有交代</strong>。
                                </p>
                            </div>
                            ...
                            <div className="border-l-4 border-[#7C3AED] bg-[#F5F3FF] rounded-r-[6px] p-3">
                                ...
                                <ul className="text-[12px] text-[#4C1D95] leading-[1.85] list-decimal pl-5 space-y-0.5">
                                    <li><strong>四層結論定稿</strong>（描述／詮釋／呼應／批判）</li>
                                    <li><strong>三欄自查</strong>（能說 / 謹慎說 / 不能說）</li>
                                    <li><strong>雷 #3 改寫練習</strong>（把「證實了」改成謹慎版）</li>
                                    <li><strong>AI-RED 敘事紀錄</strong>（若有用 AI 工具）</li>
                                </ul>
                                <p className="text-[11.5px] text-[#4C1D95] italic mt-2 pt-2 border-t border-[#7C3AED]/30">
                                    💡 評分重點：你知不知道「能說到哪、不能說到哪」——不是填滿每格，是結論的負責程度。
                                </p>
                            </div>
                    ...
                    <div className="bg-[#EFF6FF] border-2 border-[#1E40AF] rounded-[var(--radius-unified)] p-4">
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[14px] font-bold text-[#1E40AF]">複製 W15 學習紀錄 → 貼 Google Classroom</span>
                        </div>
                        <p className="text-[12px] text-[#1E3A8A] leading-relaxed mb-3">
                            包含：推論層分析／AI 檢核紀錄／能說 vs 不能說自查。
                        </p>
                        <ExportButton
                            weekLabel="W15 推論層與 AI 檢核"
                            fields={EXPORT_FIELDS}
                            buttonText="複製 W15 學習紀錄"
                        />
                    </div>
                    ...
                    <div className="bg-[var(--ink)] border-l-4 border-[var(--danger)] p-6 rounded-r-lg text-white shadow-xl">
                        ...
                        <p className="text-[var(--ink-light)] text-sm mb-4">
                            資料判斷取捨能力——面對可疑數據或結論，你能挑出真正該採納的那一份嗎？
                        </p>
    ...
    return (
        <div className="page-container animate-in-fade-slide">
            ...
            <HeroBlock
                ...
                question="我能說到哪裡？不能說到哪裡？"
                title="四層結論 · "
                accentTitle="從圖的說明到研究結論"
                todo={[
                    { label: '今天做什麼', value: '把整份研究的發現整合成一個四層結論。' },
                    { label: '為什麼做', value: '結論要回答你最初的研究問題，也要說清楚自己能說到哪、不能說到哪。' },
                    { label: '今天交什麼', value: '小組＝研究結論章初稿；個人＝W15 歷程 docx。' },
                ]}
                chain="W14 圖表和圖說都寫完了——最後一步是把所有發現整合成結論，說清楚你能說什麼、不能說什麼，給研究畫上句點。"
                meta={[
                  { label: '第一節', value: '四層結論寫作法（描述 / 詮釋 / 呼應 / 批判）+ 範例解析' },
                  { label: '第二節', value: '呼應層（連回研究問題）+ 批判層（三個研究限制）+ 完成繳交' },
                  { label: '課堂產出', value: '小組：研究結論章初稿；個人：W15 歷程 docx' },
                  { label: '前置要求', value: 'W14 圖表與圖說（四層結論要從圖的發現往上寫）' },
                ]}
            />
            ...
            {mode === 'self-study' && (
                <div className="my-5 rounded-[var(--radius-unified)] border-2 border-[#0284C7] bg-[#F0F9FF] p-4 md:p-5">
                    <p className="text-[14px] font-bold text-[#075985] mb-1">📖 自學補課路線 · W15 可「部分自學」</p>
                    <p className="text-[11.5px] text-[#0C4A6E] leading-[1.7] mb-3">
                        四層架構、名詞升級、相關 vs 因果、四層自查清單、AI 檢核紅線、雷 #3 改寫都能自學；小組「研究結論章初稿」要回組整合進報告，結論品質想被檢核可回課堂或找老師。
                    </p>
                    <ol className="text-[12px] text-[#0C4A6E] leading-[1.85] list-none m-0 p-0 space-y-1">
                        <li><strong>① 先看：</strong>Step 1（四層架構＋呼應層為什麼特別）＋ Step 3 相關 vs 因果</li>
                        <li><strong>② 做：</strong>Step 2 拿自己 W14 的描述／推論當雛型，寫四層結論初稿</li>
                        <li><strong>③ 補紀錄：</strong>四層初稿／三欄分類／自查紀錄／四層定稿／雷 #3 改寫</li>
                        <li><strong>④ 交：</strong>個人 W15 歷程 docx（四層定稿、三欄自查、雷 #3 改寫、AI-RED 如有）</li>
                        <li><strong>⑤ 需要找人：</strong>小組結論章整合回報告要回組；結論品質想被檢核回課堂或找老師</li>
                    </ol>
                    <div className="mt-3 pt-3 border-t border-[#0284C7]/30">
                        ...
                        <p className="text-[11.5px] text-[#0C4A6E] leading-[1.85] m-0">
                            ① 寫出四層結論初稿　② 用四層自查清單檢一輪、寫自查紀錄　③ 填四層結論最終版　④ 完成雷 #3 改寫　⑤ 匯出 W15 歷程 docx
                        </p>
                    </div>
                </div>
            )}
            ...
            <TaskCard
                ...
                exportReminder="小組繳交研究結論章初稿（放回研究報告）；個人繳交 W15 歷程 docx（四層結論、三欄自查、雷 #3 改寫、AI-RED 如有）。完成後到 Classroom 按「繳交」。"
            />
            ...
            {/* W15 階段紅線完整版 — 7 條 · 深度補充 */}
            <DepthBlock title="W15 研究員紅線（完整版）">
                <ResearcherRedlines mode="subset" stage="W15" />
            </DepthBlock>
        </div>
    );
};
