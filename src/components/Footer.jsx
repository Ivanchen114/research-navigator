...
export const Footer = () => {
    ...
    // Default: 學術/教學版 Footer (淺色系 · 緊湊橫向佈局，對齊黑底版階層)
    return (
        <footer className="w-full bg-[#f7f5f0] border-t border-slate-200 mt-auto relative overflow-hidden text-slate-500 font-mono">
            {/* Top thin accent line (低調，不用 cyan glow) */}
            <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-slate-300 to-transparent"></div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-2.5">
                <div className="flex flex-col md:flex-row justify-between items-center gap-3 md:gap-4">
                    ...
                    {/* Left: Branding & Core */}
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        <div className="bg-white border border-slate-200 p-2 flex items-center justify-center text-blue-600 rounded-sm shrink-0 shadow-sm">
                            <Fingerprint size={22} strokeWidth={1.5} />
                        </div>
                        <div>
                            <div className="flex items-center gap-2">
                                <h4 className="text-slate-800 font-black tracking-widest text-[13px] uppercase">AI-RED 學習框架</h4>
                                <span className="bg-blue-50 text-blue-700 border border-blue-200 text-[9px] px-1.5 py-[1px] tracking-widest rounded-sm">UPDATED · 2026.04.19</span>
                            </div>
                            <div className="flex gap-2 text-[9px] text-slate-500 tracking-[0.2em] font-bold uppercase mt-0.5">
                                <span>Ascribe</span><span className="opacity-30">/</span>
                                <span>Inquire</span><span className="opacity-30">/</span>
                    ...
                    {/* Middle / Right: School & Specs */}
                    <div className="flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-6 w-full md:w-auto border-t md:border-t-0 md:border-l border-slate-200 pt-2 md:pt-0 md:pl-6">

                        {/* School Info */}
                        <div className="flex flex-col gap-0.5">
                            <div className="flex items-center gap-2 text-slate-700 font-bold tracking-widest text-[12px]">
                                <School size={14} className="text-slate-400" /> 台北市立松山高中
                            </div>
                            <div className="text-[10px] text-slate-500 flex items-center gap-2">
                                <span className="uppercase tracking-widest">Subject: <span className="text-slate-700">研究方法與專題</span></span>
                                <span className="inline-block bg-white border border-slate-200 px-1.5 py-[1px] rounded-sm text-[8px] text-slate-500 uppercase">Required</span>
                            </div>
                        </div>

                        {/* Systems */}
                        <div className="flex flex-col gap-1 md:items-end">
                            <div className="text-[9px] text-amber-600 tracking-widest flex items-center gap-1 font-bold uppercase">
                                <Zap size={11} /> 課堂 ＋ 自學雙模式
                            </div>
                            <div className="flex gap-1 text-[8px] text-slate-600 font-bold uppercase tracking-wider">
                                <span className="bg-white border border-slate-200 px-1.5 py-[1px] rounded-sm">可投影</span>
                                <span className="bg-white border border-slate-200 px-1.5 py-[1px] rounded-sm">可手機</span>
                                <span className="bg-white border border-slate-200 px-1.5 py-[1px] rounded-sm">可回家練習</span>
                            </div>
                        </div>
